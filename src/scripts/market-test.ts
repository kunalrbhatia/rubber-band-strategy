...
async function runMarketTest() {
  try {
    ...
    if (!config.paperTrading) {
        logger.warn('WARNING: config.paperTrading is FALSE. This test will use LIVE API if you proceed.');
    ...
    for (let i = 0; i < 6; i++) {
        ...
        if (activeTrade) {
            ...
            logger.info(`Tick ${i+1}: Sell LTP: ${activeTrade.sellLeg.currentPremium}, Buy LTP: ${activeTrade.buyLeg.currentPremium}, MTM: ₹${mtm.toFixed(2)}`);
        } else {
